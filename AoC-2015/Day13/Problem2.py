...
def optimal(curr, start, mini):
    if len(visited) == len(family)-1: 
        return family[curr][start] + family[start][curr] - mini
    ...
    for next in family[curr]:
        if next not in visited:
            ans = max(ans, optimal(next, start,min(mini, family[curr][next] + family[next][curr])) + family[curr][next] + family[next][curr])
...
print(optimal(start, start, float('inf')))